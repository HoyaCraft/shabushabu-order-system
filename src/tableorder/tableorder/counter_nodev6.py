...
class TableInfoDialog(QtWidgets.QDialog):
    ...
    def onRefreshClicked(self):
        ...
        table_str = self.table_id[1:]
        self.parentWindow.ros_node.publishRefresh(int(table_str))
        self.close()
...
class CounterDP(QtWidgets.QMainWindow, ui_counter_dp.Ui_MainWindow):
    ...
    def onOkClicked(self):
        if self.listWidgetWaiting.count() > 0:
            ...
            for menu_name, qty in current_order.items():
                self.stack[table_id][menu_name] = self.stack[table_id].get(menu_name, 0) + qty
    ...
    def onGlobalGoClicked(self):
        ...
        if self._batteryLevel < 50:
            # 경고창 띄우고 이동 불가
            warn_text = (
                f"현재 배터리가 {self._batteryLevel:.0f}% 입니다.\n"
                "50% 이상에서만 이동할 수 있습니다."
            )
            self.showNonBlockingWarning("배터리 부족", warn_text)
            return
        try:
    ...
    def setAMRState(self, state: str):
        # 이미 "error"라면 다른 상태로 전환 금지
        if self._amrState == "error" and state not in ("error",):
            return
    ...
    #####################################################################
    # (수정) 경고창을 모달이 아닌 형태로 띄워서 배터리 로직 계속 동작
    #####################################################################
    def showNonBlockingWarning(self):
        """
        배터리가 30% 이하일 때 띄울 경고 메시지 박스를
        모달이 아닌 형태로 show()하여 띄운다.
        """
        msg_box = QtWidgets.QMessageBox(self)
        msg_box.setIcon(QtWidgets.QMessageBox.Warning)
        msg_box.setWindowTitle("배터리 경고")
        msg_box.setText(f"AMR 배터리가 {self._batteryLevel:.0f}% 입니다!\n조속한 충전이 필요합니다.")
        msg_box.setStandardButtons(QtWidgets.QMessageBox.Ok)
        msg_box.setModal(False)         # 모달 OFF
        msg_box.show()                  # exec_() 대신 show()

        # (옵션) 메시지박스를 참조 리스트에 넣어, GC 방지
        self._activeMessages.append(msg_box)
    ...
    def onTimer(self):
        ...
        elif self._amrState == "running":
            self._batteryLevel = max(0, self._batteryLevel - 0.5)
            self._chargingStartTime = 0
            if self._batteryLevel == 0:
                self.setAMRState("error")
            # (여기서 모달 대신 non-blocking 경고창)
            if 0 < self._batteryLevel <= 30:
                if not self._lowBatteryWarningShown:
                    self._lowBatteryWarningShown = True
                    self.showNonBlockingWarning()
            else:
                if self._batteryLevel > 30:
                    self._lowBatteryWarningShown = False
    ...
    def onTableButtonClicked(self, table_num):
        table_id = f"t{table_num}"
        if table_id not in self.stack or table_id not in self.allOrders:
            return

        earliest_time = self.earliestTimeMap.get(table_id, "-")
        total_orders = self.stack[table_id]
    ...
    def clearTableData(self, table_id):
        if table_id in self.stack:
            del self.stack[table_id]
